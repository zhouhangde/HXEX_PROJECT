<template>
  <div class="k-line-area bg-1d1d29">
    <div class="flex ft12 top_bg">
      <span class="ft16 bold flex alcenter">{{$store.state.symbol}}</span>
      <span class="flex column alcenter center">
        <span class="bold ft16" :style="{color:parseFloat(change)>0?'#27b054':'#FF4F00'}">{{close}}</span>
      ...
      <span class="flex column alcenter">
        ...
        <span
          class="bold"
          :style="{color:parseFloat(change)>0?'#27b054':'#FF4F00'}"
...
export default {
  ...
  methods: {
    ...
    connect(real) {
      //console.log("555555555555555555", real);
      //封装推送数据
      var that = this;
      //websocket
      // 打开一个 web socket
