<template>
  <div class="k-line-area bg-1d1d29">
    <div class="flex ft12 top_bg">
      ...
      <span class="flex column alcenter"><span class="bold ft16" :style="{color:parseFloat(change)>0?'#27b054':'#FF4F00'}">{{(close-0).toFixed(1)}}</span><span class="gray_color bold">≈4000.00 CNY</span></span>
      <span class="flex column alcenter"><span class="gray_color">{{$t('market.change')}}</span><span class="bold" :style="{color:parseFloat(change)>0?'#27b054':'#FF4F00'}">{{(change-0)}}%</span></span>
      <span class="flex column alcenter"><span class="gray_color">{{$t('home.high')}}</span><span class="bold_gray bold">{{(high-0).toFixed(6)}}</span></span>
      <span class="flex column alcenter"><span class="gray_color">{{$t('home.min')}}</span><span class="bold_gray bold">{{(low-0).toFixed(6)}}</span></span>
      <span class="flex column alcenter"><span class="gray_color">24h{{$t('home.volume')}}</span><span class="bold_gray bold">{{(volume-0).toFixed(2)}} {{currencyName}}</span></span>
    </div>
